...
- Name: rental-system.spec.ts >> Dashboard >> Lending ボタンが /rentals/new へ遷移する
- Location: e2e/rental-system.spec.ts:111:3
...
Locator: locator('a[href="/rentals/new"]').filter({ hasText: 'Lending' })
Expected: visible
Error: strict mode violation: locator('a[href="/rentals/new"]').filter({ hasText: 'Lending' }) resolved to 2 elements:
    1) <a href="/rentals/new" inactive-class="text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800" class="flex items-center gap-3 px-3 py-2 rounded-lg font-medium transition-colors cursor-pointer">…</a> aka getByRole('navigation').getByRole('link', { name: 'Lending' })
    2) <a data-slot="base" href="/rentals/new" class="rounded-md inline-flex items-center disabled:cursor-not-allowed aria-disabled:cursor-not-allowed disabled:opacity-75 aria-disabled:opacity-75 transition-colors py-2 text-sm gap-2 text-inverted bg-primary hover:bg-primary/75 active:bg-primary/75 disabled:bg-primary aria-disabled:bg-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary cursor-pointer font-bold px-8 shadow-lg shadow-blue-500/20">…</a> aka getByRole('main').getByRole('link', { name: 'Lending' })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for locator('a[href="/rentals/new"]').filter({ hasText: 'Lending' })
...
> 116 |     await expect(lendingButton).toBeVisible()
  ...
  178 | 
  179 |     // "Bike" カテゴリをクリック
  180 |     const bikeFilter = page.locator('button', { hasText: /^Bike$/ })
  181 |     await expect(bikeFilter.first()).toBeVisible()
  182 |     await bikeFilter.first().click()
  183 |     await page.waitForTimeout(500)
  184 | 
  185 |     const rows = page.locator('tbody tr')
  186 |     const count = await rows.count()
  187 |     if (count > 0) {
  188 |       for (let i = 0; i < count; i++) {
  189 |         const text = await rows.nth(i).textContent()
  190 |         expect(text).toContain('Bike')
  191 |       }
  192 |     }
  193 |   })
  194 | 
  195 |   test('Vehicle Details サイドバーが表示される', async ({ page }) => {
  196 |     await page.goto('/vehicles')
  197 |     await page.waitForLoadState('networkidle')
  198 |     await waitForLoadingComplete(page)
  199 | 
  200 |     // 1行目をクリック
  201 |     const firstRow = page.locator('tbody tr').first()
  202 |     await expect(firstRow).toBeVisible()
  203 |     await firstRow.click()
  204 | 
  205 |     // サイドバーが表示される
  206 |     await expect(page.getByText('Vehicle Details')).toBeVisible()
  207 |     await expect(page.locator('aside').getByText('Category')).toBeVisible()
  208 |     await expect(page.locator('aside').getByText('Status Access')).toBeVisible()
  209 |     await expect(page.locator('aside').getByText('Last Mileage')).toBeVisible()
  210 |   })
  211 | })
  212 | 
  213 | // ============================================================
  214 | // 3. Add Vehicle Test
  215 | // ============================================================
  216 | test.describe('Add Vehicle', () => {
```